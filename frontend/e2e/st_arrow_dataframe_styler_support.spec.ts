...
describe("st.dataframe supports styling and display values via Pandas Styler", () => {
  beforeEach(async ({ page }) => {
    await page.goto("http://localhost:3001/")
    await expect(page.locator(".stDataFrame")).toHaveCount(4)
  })

  test("matches all screenshots", async ({ page }) => {
    // Select all elements with stDataFrame class
    const dataFrame = await page.$$(".stDataFrame")

    await expect(dataFrame.length).toBe(4)
    ...
    for (let i = 0; i < dataFrame.length; i++) {
      expect(await dataFrame[i].screenshot()).toMatchSnapshot(
        `dataframe-styler-support-${i}.png`
      )
    }
  })
})
